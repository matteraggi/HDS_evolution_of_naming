"""
Quantitative Event Study for the Music & Pop Culture category.

Evaluates the impact of Billboard #1 artists and FIMI/Sanremo chart hits (1999-2024, US & IT)
...
Outputs:
  - dataset/processed/music_event_study_results.csv
  - docs/paper/tables/table18b_music_event_study.csv
"""
...
DATASET_PATH = os.path.join(os.path.dirname(__file__), "..", "..", "dataset", "music_events_dataset.csv")
...
OUT_RESULTS_PATH = os.path.join(os.path.dirname(__file__), "..", "..", "dataset", "processed", "music_event_study_results.csv")
TABLE_PATH = os.path.join(os.path.dirname(__file__), "..", "..", "docs", "paper", "tables", "table18b_music_event_study.csv")
...
def main():
    ...
    table_fieldnames = ["Anno", "Paese", "Brano / Evento", "Artista / Nome", "Sesso", "Freq Pre (per 100k)", "Freq Post (per 100k)", "Impatto Netto DiD (%)"]
    table_rows = []
    for r in results:
        table_rows.append({
            "Anno": r["year"],
            "Paese": r["country"],
            "Brano / Evento": r["title"],
            "Artista / Nome": r["character"],
            "Sesso": r["sex"],
    ...
    print(f"Executed Music Event Study on {len(results)} valid music events.")
