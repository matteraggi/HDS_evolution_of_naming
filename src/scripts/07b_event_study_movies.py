"""
Quantitative Event Study for the Movie & TV Series category.

Evaluates the impact of top-grossing films and major TV series (1999-2024, US & IT)
on baby name frequencies by measuring pre-event vs post-event relative frequency changes,
comparing each exposed name against the median change of all unexposed names of the same
sex/country within +/-35% of its pre-event frequency (Difference-in-Differences),
and running an overall Wilcoxon signed-rank test.

Outputs:
  - dataset/processed/movie_event_study_results.csv
  - docs/paper/tables/table18_movie_event_study.csv
"""
...
DATASET_PATH = os.path.join(os.path.dirname(__file__), "..", "..", "dataset", "movie_events_dataset.csv")
...
OUT_RESULTS_PATH = os.path.join(os.path.dirname(__file__), "..", "..", "dataset", "processed", "movie_event_study_results.csv")
TABLE_PATH = os.path.join(os.path.dirname(__file__), "..", "..", "docs", "paper", "tables", "table18_movie_event_study.csv")
...
def load_it_data():
    ...
    with open(IT_PATH, encoding="utf-8") as f:
        for r in csv.DictReader(f):
            ...
            sex = r["gender"].upper()
            year = int(r["year"])
            freq = float(r["percent"]) * 1000.0  # convert % to per 100,000 births
            data[(name_norm, sex)][year] = freq
    return data
...
def get_post_freq(time_series, event_year):
    ...
    if not post_years:
        # Fallback to event year if immediate post-years missing
...
def find_control_change(all_series, exposed_name, sex, event_year, pre_freq):
    # Find 5 non-exposed names of same sex with similar pre-event frequency (within +/- 30%)
    matches = []
...
def main():
    ...
    for ev in movie_events:
        ...
        if pre_freq is None or post_freq is None:
            # Name too rare or missing pre/post coverage
            continue
    ...
    table_fieldnames = ["Anno", "Paese", "Film / Serie TV", "Personaggio", "Sesso", "Freq Pre (per 100k)", "Freq Post (per 100k)", "Impatto Netto DiD (%)"]
    table_rows = []
    for r in results:
        table_rows.append({
            "Anno": r["year"],
            "Paese": r["country"],
            "Film / Serie TV": r["title"],
            "Personaggio": r["character"],
            "Sesso": r["sex"],
    ...
    print(f"Executed Movie Event Study on {len(results)} valid character events.")
